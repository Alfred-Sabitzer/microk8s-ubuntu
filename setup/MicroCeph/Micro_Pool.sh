...
cephfs_exists() {
  local fs="$1"
  sudo microceph.ceph fs ls 2>/dev/null | awk '{print $2}' | grep -wq -- "$fs" || return 1
}
...
create_pool() {
  ...
  if ! cephfs_exists "${pool}_fs"; then
    die "CephFS "${pool}_fs" does not exists. Skipping creation."
  fi
  echo "Creating pool '$pool' ..."

  # Create a pool using Ceph in LXD
  for host in "${HOSTS[@]}"; do
    echo "Creating LXC Pool '${pool}' on target ${host}..."
    if ! retry 5 5 sudo lxc storage create ${pool} cephfs --target ${host} source="${pool}_fs" ; then
      echo "Warning: failed to create ceph ${pool} on ${host}" >&2
    fi
  done
  retry 5 5 sudo lxc storage create ${pool} cephfs
...
main() {
    ...
    echo "Creating LXD CephFS pools..."
    create_pool "$LXD_POOL_NAME" 
