...
echo "copy files to pod '${mypod}'..."
$kubectl_cmd cp ./cephfs_test.sh -n "${namespace}" "${mypod}":/tmp/cephfs_test.sh
$kubectl_cmd exec -it -n "${namespace}" "${mypod}" -- chmod +x /tmp/cephfs_test.sh
