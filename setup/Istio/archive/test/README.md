...
- Istio enabled in MicroK8s: `sudo microk8s enable istio`
- (Optional) MetalLB enabled and configured if you want an external IP for istio-ingressgateway.
- `microk8s` CLI available in PATH (script uses `sudo microk8s kubectl`).

Quick start
1. Apply demo resources:
   sudo microk8s kubectl apply -f demo_istio.yaml
2. Apply gateway + virtualservice:
   sudo microk8s kubectl apply -f demo_istio_gateway.yaml
3. Check demo pods:
   sudo microk8s kubectl -n demo-istio get pods,svc -o wide
4. Get ingressgateway IP (requires MetalLB or external LB):
   sudo microk8s kubectl -n istio-system get svc istio-ingressgateway -o wide
...
Cleanup
- Remove demo:
  sudo microk8s kubectl delete -f demo_istio_gateway.yaml
  sudo microk8s kubectl delete -f demo_istio.yaml
  sudo microk8s kubectl delete namespace demo-istio
