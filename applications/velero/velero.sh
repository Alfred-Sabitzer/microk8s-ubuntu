...
if [ "${K8S_ENVIRONMENT}" == "test" ]; then
  ...
  velero_replica="1"
  velero_min_available="1"
else
  echo "Using Prod environment '${K8S_ENVIRONMENT}' settings for resource sizes."
  velero_replica="3"
  velero_min_available="2"
fi

# This is from https://raw.githubusercontent.com/vmware-tanzu/helm-charts/refs/heads/main/charts/velero/values.yaml
...
cat <<EOF >> "/tmp/velero-values.yaml"
  ...
  # Nothing to configure for Velero HA mode, so we will run in standalone mode.
EOF
